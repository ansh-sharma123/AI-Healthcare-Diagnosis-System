...
print("General health pipeline started...")

BASE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "data")

df = pd.read_csv(os.path.join(BASE_DIR, "general_health.csv"))

print("Dataset loaded:", df.shape)

X = df.drop("health_score", axis=1)
y_class = df["health_score"]
...
recall = recall_score(y_class_test, class_preds, average="macro")
...
with open(os.path.join(MODELS_DIR, "general_health_classifier.pkl"), "wb") as f:
    pickle.dump(classifier_pipeline, f)

with open(os.path.join(MODELS_DIR, "general_health_regressor.pkl"), "wb") as f:
